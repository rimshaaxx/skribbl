...
if (!sessionStorage.getItem('username') || !sessionStorage.getItem('roomCode')) { window.location.href = '/'; }
const myUsername = sessionStorage.getItem('username');
const myRoomCode = sessionStorage.getItem('roomCode');
const socket = io(); 

socket.emit('joinGameRoom', { roomCode: myRoomCode, username: myUsername });
...
let isDrawing = false; let amIDrawing = false; 
...
function saveState() {
    ...
    if (undoStack.length > 20) undoStack.shift(); redoStack = []; 
}

// UPGRADED: Get Mouse OR Touch Position!
...
function broadcastCanvasSync() { socket.emit('syncCanvas', { roomCode: myRoomCode, image: canvas.toDataURL() }); }
...
function startDrawing(e) {
    ...
    if (currentTool === 'fill') {
        floodFill(Math.floor(pos.x), Math.floor(pos.y), currentColor); saveState();
        socket.emit('fill', { roomCode: myRoomCode, x: Math.floor(pos.x), y: Math.floor(pos.y), color: currentColor }); return;
    }
    isDrawing = true; lastPos = pos; draw(e); 
}

function stopDrawing(e) {
    if (isDrawing && amIDrawing) { isDrawing = false; saveState(); }
}
...
function draw(e) {
    ...
    const drawData = { roomCode: myRoomCode, start: lastPos, end: pos, color: currentColor, size: currentSize, tool: currentTool, style: currentBrushStyle };
...
// MOBILE TOUCH EVENTS! (passive: false is required to stop scrolling)
...
socket.on('requestCanvasSync', () => { if (amIDrawing) broadcastCanvasSync(); });
...
// --- THE TAUNT GENERATOR ---
const taunts = ["Is that a potato?", "Squinting might help...", "My grandma draws better than this.", "What is that?!", "Abstract art at its finest.", "I think they are using the spray tool wrong.", "Ah yes... a blob.", "Staring intensely...", "Brain processing at 99%...", "I have absolutely no idea.", "They are definitely trying their best.", "Picasso is shaking right now."];
let tauntInterval;
function startTauntGenerator() {
    ...
    tauntInterval = setInterval(() => {
        ...
        setTimeout(() => { const randomTaunt = taunts[Math.floor(Math.random() * taunts.length)]; tauntText.innerText = `"${randomTaunt}"`; tauntText.style.opacity = 1; }, 500); 
    }, 6000); 
}

// HELPER: Renders the beautiful word blanks
function renderWord(wordStr) {
    const container = document.getElementById('wordDisplay'); container.innerHTML = '';
    const words = wordStr.split(' '); 
    words.forEach(w => {
        const wordDiv = document.createElement('div'); wordDiv.className = 'word-group';
        for (let i = 0; i < w.length; i++) {
            const letterSpan = document.createElement('span'); letterSpan.className = 'letter-box';
            letterSpan.innerText = w[i] === '_' ? '' : w[i]; wordDiv.appendChild(letterSpan);
...
socket.on('updatePlayers', (players) => {
    const playerGrid = document.getElementById('gamePlayerGrid'); playerGrid.innerHTML = ''; 
    ...
    players.forEach(player => {
        ...
        const card = document.createElement('div'); card.className = 'game-player-card'; card.id = `player-card-${player.id}`;
        card.innerHTML = `<img src="${avatarUrl}" alt="${player.username}"><div class="player-info"><div class="name">${player.username} <span class="drawing-icon">✏️</span></div><div class="score">Pts: ${player.score}</div></div>`;
...
socket.on('turnStarting', (data) => {
    ...
    document.getElementById('currentRound').innerText = data.round; document.getElementById('totalRounds').innerText = data.totalRounds;
    ...
    if (amIDrawing) {
        ...
        if (data.wordMode === 'normal') {
            ...
            const wordContainer = document.getElementById('wordOptions'); wordContainer.innerHTML = '';
            data.wordChoices.forEach(word => {
                const btn = document.createElement('button'); btn.className = 'word-btn'; btn.innerText = word;
                btn.onclick = () => socket.emit('wordSelected', { roomCode: myRoomCode, word: word }); wordContainer.appendChild(btn);
            });
        } else {
            document.getElementById('customWordSelection').classList.remove('hidden'); document.getElementById('customWordInput').value = '';
        }
    } else {
...
document.getElementById('submitCustomWordBtn').addEventListener('click', () => {
    ...
    if (customWord !== '') socket.emit('wordSelected', { roomCode: myRoomCode, word: customWord });
});

// 3. START DRAWING
socket.on('startDrawingPhase', (data) => {
    document.getElementById('canvasOverlay').classList.add('hidden');
    renderWord(data.blanks);
    ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, canvas.width, canvas.height); saveState();
    playSound('start'); // <--- PLAY SOUND!
});

// 4. SHOW REAL WORD TO DRAWER
socket.on('youAreDrawing', (word) => { renderWord(word); });

// 5. TIMER UPDATE
socket.on('timeUpdate', (timeLeft) => {
    const timeText = document.getElementById('timeRemaining'); const timerContainer = document.getElementById('timerContainer');
    timeText.innerText = timeLeft;
    if (timeLeft <= 10 && timeLeft > 0) {
        timerContainer.classList.add('urgent');
        playSound('tick'); // <--- PLAY TICKING SOUND!
    } else { timerContainer.classList.remove('urgent'); }
});

// 6. GAME CHAT LOGIC
const chatInput = document.getElementById('gameChatInput'); const sendChatBtn = document.getElementById('gameSendChatBtn'); const chatMessages = document.getElementById('gameChatMessages');
function sendGameMessage() {
    const text = chatInput.value.trim();
    if (text !== '') { socket.emit('sendChat', { roomCode: myRoomCode, username: myUsername, text: text }); chatInput.value = ''; }
}
sendChatBtn.addEventListener('click', sendGameMessage); chatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') sendGameMessage(); });

socket.on('chatMessage', (data) => {
    const msgDiv = document.createElement('div'); msgDiv.className = `chat-msg ${data.type === 'system' ? 'system' : ''}`;
    
    if (data.type === 'correct') {
        msgDiv.classList.add('correct'); msgDiv.innerText = data.text;
        playSound('ding'); 
    } else if (data.type === 'close') {
        msgDiv.classList.add('close'); msgDiv.innerText = data.text;
        playSound('close'); // <--- PLAY YELLOW CLOSE SOUND!
    } else if (data.type === 'system') {
        msgDiv.innerText = data.text;
        playSound('pop'); // <--- PLAY POP FOR SYSTEM MESSAGES!
    } else { 
        msgDiv.innerHTML = `<span class="author">${data.username}:</span> ${data.text}`; 
        playSound('pop'); // <--- PLAY POP FOR NORMAL CHAT!
    }
...
socket.on('gameOver', (players) => {
    ...
    players.sort((a, b) => b.score - a.score); const winner = players[0];
    ...
    if (winner && winner.score > 0) { winnerText.innerText = `🏆 ${winner.username} Wins!\n${winner.score} pts`; } 
    else { winnerText.innerText = `It's a tie! (0 pts)`; }
